...
const { buildLibraryVideoPath, localCourseLibraryRoot } = require("../config/media");
...
const mapCourseForStorage = (courseDefinition, educatorId) => ({
...
const importLocalCourses = async () => {
  ...
  for (const courseDefinition of localCourseDefinitions) {
    const mappedCourse = mapCourseForStorage(courseDefinition, educator._id);
    await Course.findOneAndUpdate(
      { title: mappedCourse.title, educator: educator._id },
      mappedCourse,
      {
        new: true,
        upsert: true,
  ...
  console.log(`Imported local courses: ${localCourseDefinitions.length}`);
